...
Independent: (https://shap.readthedocs.io/en/latest/generated/shap.maskers.Independent.html#shap.maskers.Independent)

the original SHAP only supports numpy.ndarray with dtype=float64
but we need to support numpy.ndarray with dtype=object (e.g., patch with multiple features, or remaining information like phase)
'''
...
# from ..ana_utils import MaskedModel
# from ..ana_utils._exceptions import DimensionError, InvalidClusteringError
...
class Tabular(Masker):
    ...
    def __init__(self, data, max_samples=100, clustering=None, fixed_background=None):
        ...
        max_samples : int
            ...
            than max_samples then shap.ana_utils.sample is used to subsample the dataset. The number of
        ...
        if (fixed_background is None) or any(fixed_background) == False:
        ...
        #                 "run times. Consider shap.ana_utils.sample(data, K) to summarize the background using only K samples.")
...
def _single_delta_mask(dind, masked_inputs, last_mask, data, x, noop_code):
    if dind == noop_code:
        ...
        for i in range(masked_inputs.shape[0]):  # tag 205
            masked_inputs[i, dind] = x[dind]
...
class MyIndependent(Tabular):
    ...
    def __init__(self, data, max_samples=100, fixed_background=None):
        ...
        fixed_background : None or list of bool (modified by ChenQian)
            The features in the background data that should be fixed to the values in the input data.
            This is useful when you do not care the attribution of some features and want to fix them to the value of input data.
        """
